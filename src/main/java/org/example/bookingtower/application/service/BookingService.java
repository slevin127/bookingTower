...
public class BookingService {
    ...
    /**
     * Подтверждает бронирование для указанного пользователя и слота, проверяет обоснованность слота,
     * и гарантирует, что пользователь держит слот. Он также создает запись бронирования, обновляет статус слота для забронированного,
     * и отправляет подтверждение по электронной почте.
     *
     * @param userId     the ID of the user making the booking
     * @param slotId     the ID of the calendar slot being booked
     * @param totalPrice the total price of the booking
     * @return the confirmed {@link Booking} object containing booking details
     * @throws IllegalArgumentException if the user or slot is not found
     * @throws IllegalStateException    if the slot is not held by the user or the seat is not active
     */
    public Booking confirmBooking(Long userId, Long slotId, BigDecimal totalPrice) {
        ...
        // Обновить статус слота для забронированного
        slot.book();
        calendarSlotRepository.save(slot);
